...
for i in range(len(csv_df_copy)):
    ...
    diff = (csv_df_copy.loc[i, 'Initial_target_days'] - csv_df.loc[i, 'Revised_Days_Left']).astype(int)

    csv_df_copy.loc[i, 'Trend'] = (csv_df_copy.loc[i, 'Initial_target_days'] - csv_df.loc[i, 'Revised_Days_Left']).astype(int)
